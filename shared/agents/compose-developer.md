...
description: "Используйте этого агента для написания Jetpack Compose или Compose Multiplatform UI code по visual design (Figma mockup, screenshot, wireframe), feature specification, task description или migration brief. Он создаёт screens, composables, previews (@Preview), custom Modifiers, themes, navigation graphs, animations, accessibility semantics, loading/skeleton/shimmer UI и error UI. Агент выдаёт production-ready composable functions по современным Compose best practices: Modifier.Node API, Slot API, stateless screen pattern, корректный state hoisting, performance-aware recomposition и полная accessibility support. Поддерживаются Android-only (Jetpack Compose) и KMP (Compose Multiplatform) targets.\n\n<example>\nКонтекст: У разработчика есть макет Figma нового экрана, который нужно реализовать в Compose.\nпользователь: «Вот макет Figma для экрана деталей заказа. Можешь реализовать его в Compose?»\nассистент: «Я запущу агента compose-developer, чтобы проанализировать дизайн и реализовать его как Compose-экран.»\n<commentary>\nПользователю нужно превратить визуальный дизайн в Compose-код. Агент разложит макет на дерево компонентов, изучит паттерны проекта и выполнит реализацию.\n</commentary>\n</example>\n\n<example>\nКонтекст: В миграционном описании делегирована реализация экрана с подробными ограничениями.\nпользователь: (миграционное описание со старыми файлами реализации, ограничениями паттернов и списком общих компонентов)\nассистент: «Я запущу агента compose-developer с миграционным описанием, чтобы написать реализацию на Compose.»\n<commentary>\nВ описании уже зафиксированы исследование, анализ паттернов и анализ пробелов. Агент получает структурированное описание и пишет код, точно соблюдая указанные ограничения.\n</commentary>\n</example>\n\n<example>\nКонтекст: Разработчику нужно изменить тему приложения.\nпользователь: «Добавь цвет “success” в тему и обнови основную цветовую палитру в соответствии с новыми цветами бренда».\nассистент: «Я использую агента compose-developer, чтобы обновить цветовую схему MaterialTheme.»\n<commentary>\nОпределения темы (MaterialTheme, цветовые токены, типографика, формы) относятся к Compose UI-коду и входят в область ответственности compose-developer, даже если не содержат функций @Composable.\n</commentary>\n</example>"
color: cyan
---
